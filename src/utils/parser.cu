...
graph_t *ReadGraph(char *filename) {
  ...
  if (graph->nvtxs <= 0 || graph->nedges <= 0)
    errexit("The supplied nvtxs:%" PRIDX " and nedges:%" PRIDX
            " must be positive.\n",
  ...
  for (xadj[0] = 0, k = 0, i = 0; i < graph->nvtxs; i++) {
    ...
    if (readvs) {
      ...
      if (newstr == curstr)
        errexit("The line for vertex %" PRIDX
      ...
      for (l = 0; l < ncon; l++) {
        ...
        if (newstr == curstr)
          errexit("The line for vertex %" PRIDX " does not have enough weights "
                  ...
                  i + 1, ncon);
        if (vwgt[i * ncon + l] < 0)
          errexit("The weight vertex %" PRIDX " and constraint %" PRIDX
                  " must be >= 0\n",
                  i + 1, l);
        curstr = newstr;
      }
    ...
    while (1) {
      ...
      if (readew) {
        ...
        if (ewgt <= 0)
          errexit("The weight (%" PRIDX ") for edge (%" PRIDX ", %" PRIDX
                  ") must be positive.\n",
                  ewgt, i + 1, edge);
        curstr = newstr;
      }
      ...
      if (k == graph->nedges)
        errexit("There are more edges in the file than the %" PRIDX
                " specified.\n",
                graph->nedges / 2);
  ...
  if (k != graph->nedges) {
    ...
    printf("In the first line of the file, you specified that the graph "
           "contained\n"
           "%" PRIDX " edges. However, I only found %" PRIDX
           " edges in the file.\n",
           graph->nedges / 2, k / 2);
    if (2 * k == graph->nedges) {
      printf("\n *> I detected that you specified twice the number of edges "
             "that you have in\n");
      printf("    the file. Remember that the number of edges specified in the "
             "first line\n");
      printf("    counts each edge between vertices v and u only once.\n\n");
    }
    printf("Please specify the correct number of edges in the first line of "
           "the file.\n");
    printf("-------------------------------------------------------------------"
           "-----------\n");
    exit(0);
...
graph_t *ReadGraphGR(char *filename) {
  ...
  if (sizeof(idx_t) == sizeof(uint64_t)) {
    read = fread(xadj + 1, sizeof(idx_t), graph->nvtxs,
  ...
  if (sizeof(idx_t) == sizeof(uint32_t)) {
    read = fread(adjncy, sizeof(idx_t), graph->nedges,
                 fpin); // This is little-endian data
  ...
  if (sizeEdgeTy) {
    ...
    if (sizeof(idx_t) == sizeof(uint32_t)) {
      read = fread(adjwgt, sizeof(idx_t), graph->nedges,
                   fpin); // This is little-endian data
      graph->readew = true;
      if (read < graph->nedges)
